...
--%>
        <a href="javascript:ovid()" class="btn btn-success" id="delete_selected"><i class="icon-ok"></i>批量支付</a>
       </span>
                ...
                <table class="table table-striped table-bordered table-hover" id="sample-table">
                    ...
                    <c:forEach items="${orderVOS}" var="orders">
                        <tr>
                            ...
                            <td class="td-manage">
                                <a onClick="member_go(this,'${orders.id}')" href="javascript:;" title="订单支付"
                                   class="btn btn-xs btn-success"><i class="icon-archive bigger-120"></i></a>
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
...
<script>
    ...
    function member_go(obj, id) {
        layer.confirm('确认要支付吗？', function (index) {
            $.ajax({
                url: "/admin/order",
                data: "method=orderPast&ordersId=" + id,
                type: "POST",
                success: function (result) {
                    layer.msg('支付成功!', {icon: 6, time: 1000});
                }
            });
            setTimeout("location.href = '/admin/order?method=waitCostOrder'", "1500");
        });
    }
    ...
    $("#delete_selected").click(function () {
        ...
        layer.confirm('确认要支付' + '[' + userNames + ']' + '订单吗？', function () {
            $.ajax({
                url: "/admin/order",
                data: "method=orderPast&ordersId=" + del_idstr,
                type: "POST",
                success: function () {
                    layer.msg('支付完成!', {icon: 1, time: 1000});
                }
            });
            setTimeout("location.href = '/admin/order?method=waitCostOrder'", "1500");
        });
